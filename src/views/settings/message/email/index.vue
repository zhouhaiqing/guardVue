  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" width="40%">
      <el-form :model="baseForm" :rules="rules" ref="baseForm" label-width="180px">
        ...
        <el-form-item :label="$t('page.user.roleName')" prop="role_id">
          <el-select v-model="baseForm.role_id" clearable :placeholder="$t('page.user.roleNamePlaceholder')" ref="userSelected">
    ...
    <el-dialog :title="title" :visible.sync="dialogPwdFormVisible" width="40%">
      <el-form :model="pwdForm" status-icon :rules="pwdRules" ref="pwdForm" class="demo-ruleForm" label-width="180px">
        <el-form-item :label="$t('page.user.oldPassword')" prop="old_password">
          <el-input v-model.trim="pwdForm.old_password" type="password" class="width36" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item :label="$t('page.user.newPassword')" prop="password">
          <el-input v-model.trim="pwdForm.password" type="password" class="width36" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item :label="$t('page.user.confirmPassword')" prop="confirm_password">
          <el-input v-model.trim="pwdForm.confirm_password" type="password" class="width36" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
<script>
import { userList, userDetail, createUser, updateUser, deleteUser, switchUserStatus, updatePassword } from '@/api/user'
...
export default {
    ...
    methods: {
        ...
        getOrganList() {
            organList({ organ_id: this.$store.state.user.organ_id, status: enable })
                .then(res => {
        ...
        switchStatus($event, user_id) {
            switchUserStatus({ status: $event, user_id: user_id })
                .then(res => {
                    const { code, msg } = res
                    if (code !== 0) {
                        this.$message.error(msg)
